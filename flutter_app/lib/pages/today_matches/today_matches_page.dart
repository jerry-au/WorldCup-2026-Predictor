      ...
      child: matchesAsync.when(
        ...
        data: (data) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) => _TodayMatchCard(match: data.matches[index]),
          );
        },
      ),
    );
...
class _MatchHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final stageText = [match.stage, match.groupName].where((text) => text != null && text.isNotEmpty).join(' · ');
...
class _TeamRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Expanded(child: _TeamButton(team: match.home, alignment: CrossAxisAlignment.start)),
        Padding(
        ...
        Expanded(child: _TeamButton(team: match.away, alignment: CrossAxisAlignment.end)),
      ],
    );
  }
}
    ...
    return InkWell(
      ...
      onTap: team.code.isEmpty
          ...
          : () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => TeamDetailPage(teamCode: team.code, teamName: team.nameCn?.isNotEmpty == true ? team.nameCn! : team.name),
                ),
              );
            },
      child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (prediction == null)
            ...
            Row(
              children: [
                Expanded(child: _ProbabilityItem(label: '主胜', value: prediction.win)),
                Expanded(child: _ProbabilityItem(label: '平局', value: prediction.draw)),
                Expanded(child: _ProbabilityItem(label: '客胜', value: prediction.lose)),
              ],
            ),
        ],
      ),
...
class _OddsSection extends StatelessWidget {
  ...
  const _OddsSection({required this.odds});
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              Text(
                odds == null ? '暂无数据' : '${odds!.providerCount} 家机构',
                style: TextStyle(fontSize: 12, color: Colors.grey.shade600),
              ),
            ],
          ),
